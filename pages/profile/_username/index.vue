  ...
  v-container(v-else, fluid)
    v-row
      v-col(cols='2')
        ...
        v-row(v-if='user.gender')
          v-col.pt-1.pb-0
            b Gender:
          v-col.pt-1.pb-0.text-right
            | {{ user.gender }}
        v-row(v-if='user.birthdate')
          v-col.pt-1.pb-0
            b Birthday:
          v-col.pt-1.pb-0.text-right
            | {{ new Date(user.birthdate.seconds * 1000).getFullYear() }}
        v-row(v-if='user.location')
          v-col.pt-1.pb-0
            b Location:
          v-col.pt-1.pb-0.text-right
            | {{ user.location }}
        v-row
          v-col.pt-1.pb-0
            b Join Date:
          v-col.pt-1.pb-0.text-right
            | {{ $dateFns.format(user.created.seconds * 1000, "MMM d, yyyy") }}
        v-row
          v-col(offset='3')
        ...
        v-row
          ...
          v-col
            ...
            v-row(
              ...
              v-col(cols='4')
                | {{ $dateFns.format(element.postedOn.seconds * 1000, "MMM d, hh:ss a") }}
